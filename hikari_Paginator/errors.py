...
class IncorrectDataType(PaginatorError):
    def __init__(self, name, data, var):
        super().__init__(f"Incorrect data type passed in for parameter {name}; should be {data}, not {type(var)}!")


class TooManyButtons(PaginatorError):
    def __init__(self):
        super().__init__("Too many buttons! Please remove some, or utilise the overflow row!")


class TooManyFiles(PaginatorError):
    def __init__(self):
        super().__init__("Too many files! Max 10!")
...
class BadButtons(PaginatorWarning):
    def __init__(self, msg):
        self.logger = getLogger("hikari_Paginator")
